...
#         return metrics
    ...
    def training_step(self, model: nn.Module, inputs: Dict[str, Union[torch.Tensor, Any]]) -> torch.Tensor:
        """
        Perform a training step on a batch of inputs, with additional logging for debugging gradients.
        """
        model.train()
        inputs = self._prepare_inputs(inputs)

        # 检查输入是否包含 labels（训练时应当包含）
        if "labels" not in inputs:
            logger.error("Training batch does not contain 'labels'!")
        else:
            logger.info("Training batch labels shape: {}".format(inputs["labels"].shape))

        # 计算损失，注意 compute_loss 内部调用了模型的 forward 方法（会进入 cl_forward）
        cm = self.compute_loss_context_manager() if hasattr(self, "compute_loss_context_manager") else nullcontext()
        with cm:
            loss = self.compute_loss(model, inputs)

        if self.args.n_gpu > 1:
            loss = loss.mean()  # 多GPU下取平均
        ...
        # 在反向传播后，打印分类器（classifier）中各参数的梯度
