...
#         tasks = ['STSBenchmark', 'SICKRelatedness', 'AI_detection']
...
#         return metrics
    ...
    def training_step(self, model: torch.nn.Module, inputs: dict) -> torch.Tensor:
        ...
        #             logger.info("After backward - Gradient norm for {}: {:.4f}".format(name, param.grad.norm().item()))
        #         else:
        #             logger.info("After backward - No gradient for parameter: {}".format(name))
        # else:
        #     logger.warning("Model has no 'classifier' attribute to log gradients.")

        # 可选：在梯度裁剪后再打印一次梯度（如果使用梯度裁剪）
        if self.args.max_grad_norm is not None and self.args.max_grad_norm > 0 and not self.deepspeed:
            ...
            #     for name, param in model.classifier.named_parameters():
            #         if param.grad is not None:
            #             logger.info("After clipping - Gradient norm for {}: {:.4f}".format(name, param.grad.norm().item()))
            #         else:
            #             logger.info("After clipping - No gradient for parameter: {}".format(name))
            # else:
            #     logger.warning("Model has no 'classifier' attribute to log gradients.")
        ...
        for epoch in range(epochs_trained, num_train_epochs):
            ...
            for step, inputs in enumerate(epoch_iterator):
                ...
                if (step + 1) % self.args.gradient_accumulation_steps == 0 or (
                    ...
                    #     for name, param in model.classifier.named_parameters():
                    #         if param.grad is not None:
                    #             logger.info("Before clipping - Gradient norm for {}: {:.4f}".format(name, param.grad.norm().item()))
                    #         else:
                    #             logger.info("Before clipping - No gradient for parameter: {}".format(name))
                    # else:
                    #     logger.warning("Model has no 'classifier' attribute to log gradients.")
                    # Gradient clipping
                    if self.args.max_grad_norm is not None and self.args.max_grad_norm > 0 and not self.deepspeed:
            ...
            #     for name, param in model.classifier.named_parameters():
            #         if param.grad is not None:
            #             classifier_gradients.append(param.grad.norm().item())
            #             logger.info("Epoch {} - Gradient norm for {}: {:.4f}".format(epoch + 1, name, param.grad.norm().item()))
            #     if classifier_gradients:
            #         avg_grad = sum(classifier_gradients) / len(classifier_gradients)
            #         logger.info("Epoch {} - Average classifier gradient norm: {:.4f}".format(epoch + 1, avg_grad))
            # else:
            #     logger.warning("Model has no 'classifier' attribute to log gradients.")
